...
function loadCSVData(csvFile, myLat, myLon){
Papa.parse(csvFile, {
    download: true,
    header: true,
    delimiter: "\t",
    complete: function(result) {
        ...
        result.data.forEach(function(row) { 
            ...
            console.log(lat + lon);
            if(!isNaN(lat) && !isNaN(lon))  {
                if(map.distance([myLat, myLon], [lat, lon]) <= 50*1609.34){
                    console.log(lat + " " + lon);
                    L.marker([lat, lon])
                    .addTo(map)
                    .addTo(speciesLayer)
                    .bindPopup(`Species: ${species} Staus: ${status}`);
                } 
            }
        });
    }
...
const csvFile = 'database/Database.csv'

document.getElementById("addSpecies").addEventListener("click", () => loadCSVData(csvFile, parseFloat(lat_value.value), parseFloat(long_value.value)));   